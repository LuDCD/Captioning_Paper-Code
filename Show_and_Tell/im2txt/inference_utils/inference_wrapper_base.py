...
Subclasses must implement实现 the following methods:

  build_model(): 建立推理模型 和 返回模型对象
    <1> Builds the model for inference推理 and <2> returns the model object.

  feed_image():获取编码图像并返回初始模型状态
    Takes an encoded image and returns the initial model state,
    where "state" is a numpy array whose specifics are defined by the subclass,
    e.g. concatenated LSTM state. It's assumed that feed_image() will be called
  ...
  inference_step():在一个时间步骤中获取一批输入和状态。
    返回与输入对应的softmax输出以及批次的新状态。
    ...
    Optionally also returns metadata about the current inference step,
    e.g. a serialized numpy array containing activations from a particular model layer.
...
Client usage:
  ...
  3. For each image in a batch of images:
     a) Call调用 feed_image() once to get the initial state.
...
class InferenceWrapperBase(object):
    ...
    """
        ...
        def _restore_fn(sess):
            ...
            tf.logging.info(
                "Successfully loaded checkpoint: %s", os.path.basename(checkpoint_path)
            )
